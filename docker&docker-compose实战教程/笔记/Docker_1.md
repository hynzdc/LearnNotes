...
![image-20220301215848085](https://pic-es.oss-cn-shanghai.aliyuncs.com/img/image-20220301215848085.png)

![image-20220301215901457](https://pic-es.oss-cn-shanghai.aliyuncs.com/img/image-20220301215901457.png)
...
![image-20220301215916411](https://pic-es.oss-cn-shanghai.aliyuncs.com/img/image-20220301215916411.png)
...
![image-20220301215935054](https://pic-es.oss-cn-shanghai.aliyuncs.com/img/image-20220301215935054.png)
...
![image-20220301215948325](https://pic-es.oss-cn-shanghai.aliyuncs.com/img/image-20220301215948325.png)
...
# 4.删除镜像
	docker rmi 镜像名	--------------------------  删除镜像 (下面的简写)
	docker image rm 镜像名:[tag] ------------------ 删除镜像
		-f		强制删除
~~~
...
# 1.运行容器
    ...
    --name 					别名为容器起一个名字
    -d							启动守护式容器（在后台启动容器）
    -p 							映射端口号：原始端口号		指定端口号启动  可以映射多个端口
...
# 10.数据卷(volum)实现与宿主机共享目录
	用来实现容器中的数据和宿主机中的数据进行映射（同步的）
	注意：使用容器数据卷必须在容器首次启动时设置
	docker run -v 宿主机的路径|任意别名:/容器内的路径 镜像名
		注意: 
				1.如果是宿主机路径必须是绝对路径,宿主机目录会覆盖容器内目录内容（注意这种方式会将容器路径内全部内容和清空，始终以宿主机路径为主 ）
				2.如果是别名则会在docker运行容器时自动在宿主机中创建一个目录,并将容器目录文件复制到宿主机中
				
				使用绝对路径
				docker run -d -p 8081:8080 --name hynzdc -v /home/hynS:/usr/local/tomcat/hynJ:ro b64abfdee99c
				ro代表read only
				使用别名
				docker run -v aa:/usr/local/tomcat/webapps   1.如果aa别名存在docker直接使用，aa不存在可以自动创建
				2.使用别名会保留容器路基原始内容 前提别名对应路径不能存在内容
				使用别名创建的可以查出来
				find / -name aaaa
...
![image-20220301212657136](https://pic-es.oss-cn-shanghai.aliyuncs.com/img/image-20220301212657136.png)
...
![image-20220301215341484](https://pic-es.oss-cn-shanghai.aliyuncs.com/img/image-20220301215341484.png)
...
![image-20220301215402126](https://pic-es.oss-cn-shanghai.aliyuncs.com/img/image-20220301215402126.png)
...
# 2.运行mysql服务
	...
	docker run --name mysql  -p 3306:3306 -d --restart=always mysql:tag  --重启docker后也会运行
...
# 8.如何开启docker aof持久化
	docker run --name redis02 -d -p 6378:6379 --restart=always redis:5.0.12 redis-server --appendonly yes
# 9.docker启动redis 开放端口，后台启动，指定名称，总是启动，开启持久化，映射数据文件外部
	docker run --name redis02 -d -p 6378:6379 --restart=always -v /root/redisData:/data redis:5.0.12 redis-server --appendonly yes
# 10.如何自定义配置文件启动redis
  a.如何获取redis配置文件？
  第一个方式：下载对应版本找到配置文件
  第二种方式：上传配置文件到宿主机指定目录 redisconf/redis.conf
  数据卷挂载配置启动
  docker run --name redis02 -d -p 6378:6379 --restart=always -v /redisconf/:/data redis:5.0.12 redis-server /data/redis.conf
```
...
- 启动出现如下错误
  ...
  ![image-20220301215435893](https://pic-es.oss-cn-shanghai.aliyuncs.com/img/image-20220301215435893.png)
...
![image-20220301215452425](https://pic-es.oss-cn-shanghai.aliyuncs.com/img/image-20220301215452425.png)
...
![image-20220301215507978](https://pic-es.oss-cn-shanghai.aliyuncs.com/img/image-20220301215507978.png)
...
# 3.为什么使用dockerfile来构建镜像呢？
	往往官方的景象不满足我们自己的要求，官方的镜像已经足够好了，已经适用大多数应用程序，但是如果我们要是有自己的业务，这个时候必须自定义镜像
# 4.如何通过dockerfile来构建自己的镜像
...
![image-20220526105739657](https://pic-es.oss-cn-shanghai.aliyuncs.com/study-pic/202205261057889.png)

![image-20220301215522369](https://pic-es.oss-cn-shanghai.aliyuncs.com/img/image-20220301215522369.png)

- **通过架构图可以看出通过DockerFile可以直接构建镜像**

### 9.2 Dockerfile解析过程

- 在我们build的时候会把所有的context目录发送给Docker Server引擎
- 对于本次build用不到的文件我们可以放到.dockerIgnore文件里
- DockerFIle就是一个简单文件，不要想的很高端
  - 要求一行只能存在一条完整的指令
  - 每读一行的时候都会构建一个临时镜像
  - 最后一条语句执行完会构造一个最终的镜像，我们只能看到最终的镜像
  - 临时镜像是为了加快构建的速度，如果不想加缓存就加-no-cache

![image-20220301215540503](https://pic-es.oss-cn-shanghai.aliyuncs.com/img/image-20220301215540503.png)

### 9.3 Dockerfile的保留命令

```dockerfile
1.每条指令都必须大写，且后面都必须至少跟一个参数
2.指令从上到下按顺序执行
3.#表示注释
4.每条指令都会创建一个新的镜像层，并对镜像进行提交
```

注：单纯的docker run centos 如果没有任何阻塞的应用，他先自己启动然后就会自己关机
...
#### 9.3.12 CMD和ENTRYPOINT的区别

- cmd里面的命令如果在运行容器时候进行覆盖：

  docker run 镜像:版本号  覆盖自己定义的命令 ls/

- Entrypoint 里面 如果在运行容器时进行覆盖：

  docker run --entrypoint=覆盖指令 镜像:版本号 传递参数

  一般是搭配着用

  ![image-20220527090244931](https://pic-es.oss-cn-shanghai.aliyuncs.com/study-pic/202205270902012.png)

- Entrypoint 用来书写容器固定的指令
- cmd用来给Entrypoint传递参数，配合使用时必须使用json数组的语法

### 9.4 Dockerfile构建springboot项目部署

##### 1.准备springboot可运行项目

![image-20220301215602349](https://pic-es.oss-cn-shanghai.aliyuncs.com/img/image-20220301215602349.png)

##### 2.将可运行项目放入linux虚拟机中

![image-20220301215617062](https://pic-es.oss-cn-shanghai.aliyuncs.com/img/image-20220301215617062.png)

![image-20220527121633821](https://pic-es.oss-cn-shanghai.aliyuncs.com/study-pic/202205271216912.png)
...
![image-20220527121848411](https://pic-es.oss-cn-shanghai.aliyuncs.com/study-pic/202205271218451.png)
...
![image-20220301215631944](https://pic-es.oss-cn-shanghai.aliyuncs.com/img/image-20220301215631944.png)
...
spring:
  datasource:
    ...
    username: root
    password: root
    url: jdbc:mysql://mysql:3306/emp?characterEncoding=utf8&connectTimeout=2000&allowPublicKeyRetrieval=true&socketTimeout=3000&autoReconnect=true&useUnicode=true&useSSL=false&serverTimezone=UTC
    driver-class-name: com.mysql.cj.jdbc.Driver
mybatis-plus:
  mapper-locations: classpath:mapper/*.xml
  configuration:
    log-impl: org.apache.ibatis.logging.stdout.StdOutImpl
  type-aliases-package: com.hyn.docker.entity
```
...
- 默认的docker在创建容器的时候，将所有容器都连接到docker0网桥上，默认在docker0网桥的容器都可以使用容器内ip进行通信
- 默认的docker在创建容器的时候，将所有容器都连接到docker0网桥上，默认在docker0网桥的容器都可以使用容器的名称作为ip进行通信。注意：使用容器的名称必须自定义网桥不能默认使用docker0
...
![image-20220301215650027](https://pic-es.oss-cn-shanghai.aliyuncs.com/img/image-20220301215650027.png)
...
# 4.加入指定的网络
- docker network connect 加入网络的名称 容器名称｜id
```
...
root@f8682db35dd7:/# curl http://nginx001   既可以使用ip地址进行通讯也可以使用容器名称进行通信
<!DOCTYPE html>
<html>
<head>
...
`数据卷定义：`用来实现宿主机和容器之间文件目录映射同步

![image-20220526085141383](https://pic-es.oss-cn-shanghai.aliyuncs.com/study-pic/202205260851622.png)
...
# 2.使用别名的方式
	docker run -v aa:/usr/local/tomcat/webapps ..tomcat:8.0
	注意bb代表数据卷的别名可以存在也可以不存在，不存在的时候docker会自动创建
	第一次使用别名的时候会将容器中的原始数据保留下来，使用绝对路径不会保留容器中的原始数据
# 3.别名到底是什么？
	别名是docker自身维护的数据卷
# 4.查看docker维护的所有数据卷
	docker volume ls
...
```markdown
# 1.现有docker进行项目部署存在的问题
	为了完成一个完整的项目，势必会用到n多个容器配合完成项目中的业务开发，一旦引入n多个容器，n多个容器之间就会形成依赖，也就意味着某个容器必须先运行其他容器后才能正常运行
	因次容器的编排显得至关重要，
# 2.现在这种方式使用容器，没有办法站在项目的角度，没有办法将一个项目用到的一组容器划分到一起，日后难点在于项目的多服务器部署
	项目角度管理容器
	Docker Compose就能解决这些问题
```
...
`Compose` 恰好满足了这样的需求。它允许用户通过一个单独的 `docker-compose.yml` 模板文件（YAML 格式）来定义一组相关联的应用容器为一个项目（project）。就是定义项目用到的多个容器

`Compose` 中有两个重要的概念：

- 服务 (`service`)：一个项目中的某一个容器我们可以称之为服务，实际上可以包括若干运行相同镜像的容器实例。
...
```markdown
# 1.快速编排
	站在项目角度将一组相关联的容器整合在一起，对这组容器进行指定顺序启动
# 2.对于多个docker容器管理
# 3.如何使用docker-compose项目
	docker-compose.yml就是应用（project），应用（project）就是docker-compose
...
```bash
1.下载安装docker-compose脚本
2.放入usr/local/bin
3.sudo chmod -x /usr/local/docker-compose
```
...
- 具体项目里要用到什么服务直接在docker-compose.yml里面配置就好了，也不用安装镜像，它会自动pull，,很方便

```markdown
# 1.docker-compose第一个案
	a.创建一个项目mkdir ems
	b.在ems创建一个docker-compose.yml模版文件 
	c.version 是compose和docker版本的对应关系
```

![image-20220527212826087](https://pic-es.oss-cn-shanghai.aliyuncs.com/study-pic/202205272128190.png)
...
services:
  tomcat: 
    ...
    ports: #代表宿主机和容器中端口的映射 -p
      - 8080:8080
  tomcat01:
    container_name: tomcat01 #代表指定一个名称 类似于--name 推荐使用默认的名称
    image: tomcat:latest
    ports:
      - 8081:8080
  redis:
...
[root@centos ~]# docker-compose down               //把所有的缓存清空
```
...
指定 `Dockerfile` 所在文件夹的路径（可以是绝对路径，或者相对 docker-compose.yml 文件的路径）。 `Compose` 将会利用它自动构建这个镜像，然后使用这个镜像。（在启动容器之前根据dockerfile构建镜像，然后根据镜像启动容器）
...
![image-20220528102836946](https://pic-es.oss-cn-shanghai.aliyuncs.com/study-pic/202205281028063.png)
...
用来指定docker服务会随着docker引擎的重启而重启。--restart = always
...
services:
  ...
  apps:
    ...
    build:
      ...
      dockerfile: Dockerfile #用来指定dockerfile的文件名字
    ports:
      - "8081:8989"
    networks:
      - dangdang
  mysql:
    ...
    networks:
      - dangdang
#    environment:
#      - "MYSQL_ROOT_PASSWORD=root"  不安全
    # command: ["test.jar"] 用来覆盖容器启动之后的默认指令
    depends_on:
      - tomcat #书写依赖的服务名
volumes:
  aa:
...
```yaml
docker-compose up ;对项目中的所有容器进行一个启动
docker-compose up  id : 对指定容器进行启动
```
...
![image-20220301215719595](https://pic-es.oss-cn-shanghai.aliyuncs.com/img/image-20220301215719595.png)
